...
describe("POST /users", function () {
  ...
  test("doesn't work for regular: create admin", async function () {
    const resp = await request(app)
        .post("/users")
        .send({
          username: "u-new",
...
describe("GET /users/:username", function () {
  test("works for same user logged in", async function () {
    const resp = await request(app)
        .get(`/users/u1`)
        .set("authorization", `Bearer ${u1Token}`);
    expect(resp.body).toEqual({
  ...
  test("works for admin to different user", async function () {
    const resp = await request(app)
        .get(`/users/u1`)
        .set("authorization", `Bearer ${uAdminToken}`);
    expect(resp.body).toEqual({
...
describe("PATCH /users/:username", () => {
  ...
  test("works for same users logged in", async function () {
    const resp = await request(app)
        .patch(`/users/u1`)
        .send({
          firstName: "New",
  ...
  test("doesn't work for same users logged in", async function () {
    const resp = await request(app)
        .patch(`/users/u2`)
        .send({
          firstName: "New",
